...
OMX_ERRORTYPE OMXMaster::makeComponentInstance(
    ...
    OMX_ERRORTYPE err =
        mPlugin->makeComponentInstance(name, callbacks, appData, component);
    return err;
#if 0
    Mutex::Autolock autoLock(mLock);
...
OMX_ERRORTYPE OMXMaster::destroyComponentInstance(
        OMX_COMPONENTTYPE *component) {
    return mPlugin->destroyComponentInstance(component);
#if 0
    Mutex::Autolock autoLock(mLock);
...
OMX_ERRORTYPE OMXMaster::getRolesOfComponent( const char *name, void *roles) {
#if 0
    Mutex::Autolock autoLock(mLock);
